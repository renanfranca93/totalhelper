...
if (!isset($_SESSION['loggedin'])) {
    ...
    if(isset($_POST['search']) && $_POST['search']!=''){
        ...
        }else header("Location: material.php?offset=0");
    }

    //PAGINACAO

    $resTotal = mysqli_query($con,'SELECT count(c.nome), c.tipo_pagamento
    FROM clients c
    LEFT JOIN payments p ON c.cod = p.id_cliente
    WHERE c.valor_inicial-(select sum(valor) from payments where id_cliente = c.cod) > 0
    OR (select sum(valor) from payments where id_cliente = c.cod) IS NULL
    AND c.valor_inicial IS NOT NULL
    AND c.tipo_pagamento = 3');
...
<body>
  ...
  <div class="sidebarTitle logoPadding"><img src="./assets/images/icon.png" height="45px"></div>
      ...
      <a href="financial.php?offset0"><div class="sidebarItem inactiveItem"><i class="fas fa-hand-holding-usd "></i> </div></a>
      <a href="material.php"><div class="sidebarItem activeItem"><i class="fas fa-box "></i> </div></a>
        ...
        <div class="row">
            <div id="content" class="col" >
                <div id="bar" class="row d-md-block d-none ">
                    <!-- BUSCA USUARIO -->
                        <div class="searchBox">
                            <form class="form-inline" method='POST' action='financial.php'>
                                <input name="search" class="form-control form-control-sm mr-sm-2" type="search" placeholder="Digite o item" aria-label="Search">
                                <button class="btn btn-sm btn-outline-secondary text-center secondColor" type="submit"><i class="fa fa-search"></i></button>
                                &nbsp;<button class="btn btn-sm btn-outline-secondary text-center secondColor" type="button">Entrada</button>
                                &nbsp;<button class="btn btn-sm btn-outline-secondary text-center secondColor" type="button">Saída</button>
                                &nbsp;<button class="btn btn-sm btn-outline-secondary text-center secondColor" type="button">Novo item</button>
                            </form>
                        </div>
                        ...
                        <table class="table table-striped">
                            <thead>
                              <tr>
                                ...
                                <!-- <th class='d-md-inline d-none'>Inicial</th>
                                <th class='d-md-inline d-none'>Restante</th> -->
                                <th>Quantidade</th>
                            ...
                            if (isset($_POST['search'])){
                                $select_query = "select * from itens";
                                // "SELECT c.cod, c.tipo_pagamento, c.nome, c.valor_inicial, c.valor_inicial-(select sum(valor) from payments where id_cliente = c.cod) as a_pagar, (select data from payments where id_cliente = c.cod ORDER BY data DESC LIMIT 1) as ultimo_pagamento
                                // FROM clients c
                                // LEFT JOIN payments p ON c.cod = p.id_cliente
                                // WHERE c.valor_inicial-(select sum(valor) from payments where id_cliente = c.cod) > 0
                                // OR (select sum(valor) from payments where id_cliente = c.cod) IS NULL
                                // AND c.valor_inicial IS NOT NULL
                                // AND c.tipo_pagamento = 3
                                // AND nome like '%".$_POST['search']."%'
                                // OR cpf like '%".$_POST['search']."%'
                                // GROUP BY c.cod
                                // ORDER BY (select data from payments where id_cliente = c.cod ORDER BY data DESC LIMIT 1) ASC
                                // ";
                                $showPag = 'none';
                                ...
                                $select_query = $select_query = "select * from itens";
                                // "SELECT c.cod, c.tipo_pagamento, c.nome, c.valor_inicial, c.valor_inicial-(select sum(valor) from payments where id_cliente = c.cod) as a_pagar, (select data from payments where id_cliente = c.cod ORDER BY data DESC LIMIT 1) as ultimo_pagamento
                                // FROM clients c
                                // LEFT JOIN payments p ON c.cod = p.id_cliente
                                // WHERE c.valor_inicial-(select sum(valor) from payments where id_cliente = c.cod) > 0
                            ...
                            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {                              
                                ...
                                // $ultimo_pagamento = date_create($row['ultimo_pagamento']);
                                

                                // $dateTest = new DateTime();
                                // $days = round(($ultimo_pagamento->format("U") - $dateTest->format("U")) / 86400);
                                // $redPayClass = "";
                                // if($days < -31) $redPayClass = "style='color:red; font-weight:bold'";

                                echo "<tr>";
                                echo "<th scope='col' class='mediumfont'>" .$row['name'] . "</th>";
                                echo "<th scope='col' class='mediumfont'>-</th>";
                                // echo "<td scope='col' class='d-md-inline d-none'>". $row['valor_inicial'] ."</th>";

                                // if(is_null($row['a_pagar'])){
                                  // echo "<td scope='col' class='d-md-inline d-none'>". $row['valor_inicial'] ."</th>";
                                // }else{
                                  // echo "<td scope='col' class='d-md-inline d-none'>". $row['a_pagar'] ."</th>";
                                // }

                                
                                // if(is_null($row['ultimo_pagamento'])){
                                //   echo "<td scope='col'> - </th>";
                                // }else{                                  
                                //   echo "<td scope='col' ".$redPayClass.">". date_format($ultimo_pagamento, 'd/m/Y') ."</th>";
                                  
                                // }
                                
                                // echo "<td class='align-middle' data-toggle='modal' data-target='#modalPagamentos'><a href='#' onclick=getUserPayments('1')><i class='fas fa-edit'></a></td>";
                            }
                            ...
                            <a href="financial.php?offset=<?php echo $offset-10 ?>">
