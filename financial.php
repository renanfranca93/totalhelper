...
if (!isset($_SESSION['loggedin'])) {
    ...
    if(isset($_POST['search']) && $_POST['search']!=''){
        ...
        }else header("Location: financial.php?offset=0");
    }
...
<body>
  ...
  <div class="sidebarTitle logoPadding"><img src="./assets/images/icon.png" height="45px"></div>
      ...
      <a href="financial.php?offset0"><div class="sidebarItem activeItem"><i class="fas fa-hand-holding-usd "></i> </div></a>
                    ...
                    <!-- BUSCA USUARIO -->
                        <div class="searchBox">
                            <form class="form-inline" method='POST' action='financial.php'>
                                <input name="search" class="form-control form-control-sm mr-sm-2" type="search" placeholder="Data inicial" aria-label="Search">
                                <input name="search" class="form-control form-control-sm mr-sm-2" type="search" placeholder="Data final" aria-label="Search">
                                <button class="btn btn-sm btn-outline-secondary text-center secondColor" type="submit"><i class="fa fa-search"></i></button>
                                &nbsp;<button class="btn btn-sm btn-outline-secondary text-center secondColor" type="button">Entrada</i></button>
                                &nbsp;<button class="btn btn-sm btn-outline-secondary text-center secondColor" type="button">Saída</i></button>
                                &nbsp;&nbsp;&nbsp;<h6>Saldo do período R$ 5.000,00 | </h6>
                                &nbsp;<h6>Caixa R$ 45.000,00</h6>
                            </form>
                        </div>
                ...
                <div class="floatLeft" class="row">
                        <table class="table table-striped">
                            ...
                            if (isset($_POST['search'])){
                                $select_query = "SELECT c.cod, c.tipo_pagamento, c.nome, c.valor_inicial, c.valor_inicial-(select sum(valor) from payments where id_cliente = c.cod) as a_pagar, (select data from payments where id_cliente = c.cod ORDER BY data DESC LIMIT 1) as ultimo_pagamento
                                FROM clients c
                                LEFT JOIN payments p ON c.cod = p.id_cliente
                                WHERE c.valor_inicial-(select sum(valor) from payments where id_cliente = c.cod) > 0
                                OR (select sum(valor) from payments where id_cliente = c.cod) IS NULL
                            ...
                            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {                              
                                ...
                                echo "<td class='align-middle' data-toggle='modal' data-target='#modalPagamentos'>-</td>";
                                echo "<td class='align-middle' data-toggle='modal' data-target='#modalPagamentos'>-</td>";
                            }
